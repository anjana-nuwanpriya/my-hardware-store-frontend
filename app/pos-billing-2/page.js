...
export default function POSBilling2Page() {
  ...
  const generateBillNumbers = async () => {
    try {
      ...
      if (orders.length > 0) {
        ...
        const orderNumber = lastOrder.order_number || 'POS-000000';
        const lastNumber = parseInt(orderNumber.split('-')[1]) || 0;
        setBillNumber(`POS-${String(lastNumber + 1).padStart(6, '0')}`);
        setRefNumber(`POS-${String(lastNumber).padStart(6, '0')}`);
      } else {
        setBillNumber('POS-000001');
        setRefNumber('POS-000000');
      }
    } catch (error) {
      setBillNumber('POS-000001');
      setRefNumber('POS-000000');
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const orderData = {
        ...
        payment_method: 'cash',
        payment_status: 'paid',
        notes: description || 'POS Billing 2 Sale'
      };
      ...
      printDocument.write(`
        ...
        <body>
          ...
          <div class="info">
            ...
            <div><span>Bill #:</span><span>${order.order_number}</span></div>
          ...
          <div class="footer">
            ...
            <p>Powered by Hardware Shop System</p>
          </div>
        </body>
        </html>
      `);
        ...
        <div className="max-w-6xl mx-auto">
          <div className="bg-white rounded-[32px] shadow-lg p-10">
            ...
            <div className="flex gap-3 mb-6">
                ...
                {showDropdown && (
                  <div className="absolute z-50 w-full mt-2 bg-white border-2 border-gray-300 rounded-2xl shadow-2xl max-h-80 overflow-y-auto">
                    {filteredProducts.map((product, index) => (
                      <div key={product.id} onClick={() => selectProduct(product)} className={`px-6 py-3 cursor-pointer transition-colors ${index === selectedIndex ? 'bg-blue-100 border-l-4 border-blue-500' : 'hover:bg-gray-50'} ${index !== filteredProducts.length - 1 ? 'border-b border-gray-200' : ''}`}>
                        <div className="flex items-center justify-between">
                          ...
                          <div className="ml-4 text-right">
                            <p className="font-bold text-blue-600">Rs {parseFloat(product.selling_price || 0).toFixed(2)}</p>
              ...
              <select value={selectedStore?.id} onChange={(e) => setSelectedStore(stores.find(s => s.id === parseInt(e.target.value)))} className="px-6 py-3 bg-white border border-gray-300 rounded-full text-sm focus:outline-none focus:border-blue-400">
                {stores.map(store => <option key={store.id} value={store.id}>{store.name}</option>)}
              </select>
              <select value={selectedCustomer?.id || ''} onChange={(e) => setSelectedCustomer(customers.find(c => c.id === e.target.value))} className="px-6 py-3 bg-white border border-gray-300 rounded-full text-sm focus:outline-none focus:border-blue-400">
                ...
                {orderItems.length === 0 ? (
                  ...
                  <div className="space-y-1">
                    {orderItems.map((item, index) => (
                      <div key={index} onClick={() => setSelectedRow(index)} className={`grid grid-cols-12 gap-2 px-2 py-2 rounded-lg cursor-pointer transition-colors ${selectedRow === index ? 'bg-blue-100 border-2 border-blue-400' : 'hover:bg-gray-50'}`}>
                        ...
                        <input type="text" value={item.batch_no} onChange={(e) => updateItem(index, 'batch_no', e.target.value)} className="col-span-2 text-sm px-2 bg-transparent focus:outline-none focus:bg-white focus:border focus:border-blue-300 rounded" placeholder="-" />
                        <input type="number" value={item.unit_price} onChange={(e) => updateItem(index, 'unit_price', parseFloat(e.target.value) || 0)} className="col-span-1 text-sm text-right px-2 bg-transparent focus:outline-none focus:bg-white focus:border focus:border-blue-300 rounded" step="0.01" />
                        <input type="number" value={item.quantity} onChange={(e) => updateItem(index, 'quantity', parseInt(e.target.value) || 1)} className="col-span-1 text-sm text-center px-2 bg-transparent focus:outline-none focus:bg-white focus:border focus:border-blue-300 rounded" min="1" />
                        <input type="number" value={item.discount_percentage} onChange={(e) => updateItem(index, 'discount_percentage', parseFloat(e.target.value) || 0)} className="col-span-1 text-sm text-right px-2 bg-transparent focus:outline-none focus:bg-white focus:border focus:border-blue-300 rounded" step="0.01" />
            ...
            <div className="grid grid-cols-2 gap-6 mb-6">
              <div className="space-y-3">
                <textarea value={description} onChange={(e) => setDescription(e.target.value)} placeholder="Description" className="w-full px-6 py-3 bg-white border border-gray-300 rounded-2xl text-sm focus:outline-none focus:border-blue-400 focus:ring-2 focus:ring-blue-200 resize-none" rows="3" />
                <select value={selectedStaff?.id || ''} onChange={(e) => setSelectedStaff(staff.find(s => s.id === e.target.value))} className="w-full px-6 py-3 bg-white border border-gray-300 rounded-full text-sm focus:outline-none focus:border-blue-400">
            ...
            <div className="grid grid-cols-5 gap-3">
              ...
              <button onClick={handlePrint} className="py-4 bg-gradient-to-r from-cyan-400 to-blue-500 text-white rounded-full font-medium text-sm shadow-md hover:shadow-lg hover:from-cyan-500 hover:to-blue-600 transition-all">&lt; F2 &gt; Print</button>
            </div>
          </div>
        </div>
      </div>
