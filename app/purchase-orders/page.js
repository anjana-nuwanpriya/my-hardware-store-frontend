...
export default function PurchaseOrderPage() {
  ...
  const fetchNextPONumber = async () => {
    try {
      ...
      setFormData(prev => ({ ...prev, po_number: res.data.poNumber }));
    } catch (error) {
  ...
  const handleDiscountPercentChange = (index, percent) => {
    ...
    const gross = parseFloat(newItems[index].quantity || 0) * parseFloat(newItems[index].unit_cost || 0);
  ...
  const calculateLineTotal = (index, itemsArray) => {
    ...
    const gross = parseFloat(item.quantity || 0) * parseFloat(item.unit_cost || 0);
  ...
  total: totals.net_total,   // <—— most important, your DB needs this!
  items: validItems
});
              ...
              <tbody>
                {items.map((item, index) => (
                  <tr key={index} className="border-b border-gray-200">
                    ...
                    <td className="p-2">
                      <input type="number" step="0.01" value={item.unit_cost} readOnly className="w-24 px-2 py-1 border border-gray-300 rounded bg-gray-50 text-sm" />
                    </td>
                    <td className="p-2">
